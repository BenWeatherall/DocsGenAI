...
class TestCLI:
    ...
    def test_main_success(self, mock_validate_results, mock_print_summary,
                         mock_generate_doc, mock_build_tree, mock_validate_path,
        ...
        mock_generate_doc.assert_called_once_with(mock_tree, '/test/path', False)
        mock_print_summary.assert_called_once_with(mock_tree, False)
        mock_validate_results.assert_called_once_with(mock_tree, False)

    @patch('genai_docs.cli.config')
    @patch('genai_docs.cli.setup_logging')
    def test_main_invalid_project_path(self, mock_setup_logging, mock_config):
    ...
    @patch('genai_docs.cli.setup_logging')
    @patch('genai_docs.cli.validate_project_path')
    def test_main_tree_building_failure(self, mock_validate_path, mock_setup_logging, mock_config):
        ...
        # Mock command line arguments
        with patch.object(sys, 'argv', ['genai-docs', '/test/path']):
            with patch('genai_docs.cli.build_and_validate_tree', side_effect=ValueError("No modules found")):
                result = main()
    ...
    @patch('genai_docs.cli.validate_project_path')
    @patch('genai_docs.cli.build_and_validate_tree')
    def test_main_keyboard_interrupt(self, mock_build_tree, mock_validate_path,
                                   mock_setup_logging, mock_config):
    ...
    @patch('genai_docs.cli.print_summary')
    @patch('genai_docs.cli.validate_results')
    def test_main_with_options(self, mock_validate_results, mock_print_summary,
                              mock_generate_doc, mock_build_tree, mock_validate_path,
                              mock_setup_logging, mock_config):
        ...
        with patch.object(sys, 'argv', ['genai-docs', '/test/path', '--verbose', '--dry-run', '--output', '/output/path', '--model', 'test-model']):
            result = main()
        ...
        mock_generate_doc.assert_called_once_with(mock_tree, '/test/path', True)  # dry_run=True
        mock_print_summary.assert_called_once_with(mock_tree, True)  # dry_run=True
        mock_validate_results.assert_called_once_with(mock_tree, True)  # dry_run=True

    @patch('genai_docs.cli.config')
    def test_main_missing_api_key(self, mock_config):
        """Test main execution with missing API key."""
        # Mock config to simulate missing API key
        mock_config.load_from_environment.return_value = None
        mock_config.validate.side_effect = ValueError("GOOGLE_API_KEY environment variable is required")
    ...
    @patch('genai_docs.cli.print_summary')
    @patch('genai_docs.cli.validate_results')
    def test_main_general_exception(self, mock_validate_results, mock_print_summary,
                                   mock_generate_doc, mock_build_tree, mock_validate_path,
                                   mock_setup_logging, mock_config):
