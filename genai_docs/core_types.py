...
class ModuleNode(BaseModel):
    ...
    # Cycle handling
    cycle_group: list["ModuleNode"] | None = None
    ...
    def get_module_name(self) -> str:
        ...
        if self.is_root:
            return self.name
        # TODO: This will be enhanced when we build the full module tree
        return self.name
    ...
    def __repr__(self) -> str:
        ...
        state = (
            f"({self.documentation_state})"
            if self.documentation_state != "pending"
            else ""
        )
...
class DependencyGraph(BaseModel):
    ...
    root_node: ModuleNode | None = None
    ...
    def get_node(self, path: str) -> ModuleNode | None:
        """Returns a node by its path."""
        return self.nodes.get(path)

    def get_node_by_name(self, name: str) -> ModuleNode | None:
...
class DocumentationContext(BaseModel):
    ...
    def get_dependency_context_string(self, dependencies: list[ModuleNode]) -> str:
        ...
        # Build dependency context dictionary
        dep_context_dict: dict[str, str] = {}
        for dep in dependencies:
            dep_name = dep.get_module_name()
            dep_context = self.get_module_context(dep_name)

            # Combine summary and interface if available
            parts = []
            if dep_context["summary"]:
                parts.append(dep_context["summary"])
            if dep_context["interface"]:
                parts.append(f"Interface: {dep_context['interface']}")

            if parts:
                dep_context_dict[dep_name] = " ".join(parts)

        if not dep_context_dict:
            return ""

        # Load and render Jinja template
        template_dir = Path(__file__).parent / "templates"
        env = Environment(loader=FileSystemLoader(template_dir))
        template = env.get_template("dependency_context.j2")
        return template.render(dependencies=dep_context_dict)
